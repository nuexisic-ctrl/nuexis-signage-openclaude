...
export default function AssetClient({
  ...
  const refreshData = useCallback(async (targetFolderId: string | null = activeFolder?.id || null) => {
    ...
    try {
      const [foldersRes, filesRes] = await Promise.all([
        ...
        fetchFolderFiles(teamSlug, targetFolderId)
      ])
      ...
      if (filesRes.success && filesRes.files) {
        setFilesCache(prev => ({
          ...prev,
          [targetFolderId || 'root']: filesRes.files as Asset[]
        }))
      }
      startTransition(() => {
        router.refresh()
      ...
      console.error('[Assets] Error during refreshData:', err)
    } finally {
      setIsRefreshing(false)
    }
  }, [activeFolder, teamId, teamSlug, supabase, router])

  const handleRefresh = useCallback(async () => {
    if (isRefreshing) return
    await refreshData(activeFolder?.id || null)
    ...
    try {
      if (folderId) {
        ...
        if (folderCheckError || !folderExists) {
          toast.warning(t('This folder no longer exists or was recently moved. Refreshing folder structure...'))
          await handleRefresh()
          return
        }
      }
  ...
  const navigateToFolder = useCallback((targetFolder: Asset | null, pathStr: string) => {
    ...
    window.history.pushState(null, '', `/customer/${teamSlug}/assets${queryParam}`)
  }, [teamSlug])
  ...
  // Keep browser URL path query parameter in sync with folder structure
  useEffect(() => {
    const params = new URLSearchParams(window.location.search)
    const pathParam = params.get('path') || '/'
    const decodedParam = decodeURIComponent(pathParam)
    const decodedCurrent = decodeURIComponent(currentPathString)
    if (decodedParam !== decodedCurrent) {
      const queryParam = currentPathString === '/' ? '' : `?path=${currentPathString}`
      window.history.replaceState(null, '', `/customer/${teamSlug}/assets${queryParam}`)
    }
  }, [currentPathString, teamSlug])

  // Monitor active folder existence and navigate to closest parent if deleted
  useEffect(() => {
    if (!activeFolder) return
    const exists = folders.some(f => f.id === activeFolder.id && f.mime_type === 'application/x-folder')
    if (!exists) {
